...
for noise in [1e-5,1e-3,1e-1]:
    prob = Rosenbrock(n=n, noise_level=noise, noise_type=noise_type)
    ...
    for seed in tqdm(range(reps), desc="SPSA reps"):
        ...
        file = open(f"Pickle_files/Rosenbrock_{noise_type}_{n}_{noise}_SPSA.pickle", "wb")
...
for noise in [1e-5,1e-3,1e-1]:
    ...
    for seed in tqdm(range(reps), desc="ImFil reps"):
        ...
        file = open(f"Pickle_files/Rosenbrock_{noise_type}_{n}_{noise}_ImFil.pickle", "wb")
...
for noise in [1e-5,1e-3,1e-1]:
    ...
    for seed in tqdm(range(reps), desc="NOMAD reps"):
        ...
        file = open(f"Pickle_files/Rosenbrock_{noise_type}_{n}_{noise}_NOMAD.pickle", "wb")
